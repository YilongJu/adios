...
def main():
    ...
    if args.wandb:
        ...
        ckpt_auroc = ModelCheckpoint(monitor='val_auroc', dirpath=chkt_dir,
        ...
        callbacks.append(ckpt_auroc)
        ckpt_loss = ModelCheckpoint(monitor='val_loss', dirpath=chkt_dir,
                                     save_top_k=5, mode='min',
                                     filename=f"{args.name}-{wandb_logger.version}-" + '{epoch:02d}-{val_loss:.4f}',
                                     auto_insert_metric_name=True, every_n_epochs=1)
        callbacks.append(ckpt_loss)
