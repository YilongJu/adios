...
class BYOL_ADIOS_1D(BaseMomentumADIOSModel):
    def __init__(
        self,
        output_dim: int,
        proj_hidden_dim: int,
    ...
    def add_model_specific_args(parent_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        parent_parser = super(BYOL_ADIOS_1D, BYOL_ADIOS_1D).add_model_specific_args(parent_parser)
    ...
    def adios_forward(self, batch: Sequence[Any], forward_type: str='inpaint') -> torch.Tensor:
        ...
        for k, mask in enumerate(torch.chunk(masks, self.N, dim=1)):
            ...
            if forward_type == 'mask':
                mask_penalty = mask.sum([-1, -2]) / mask_n_ele
