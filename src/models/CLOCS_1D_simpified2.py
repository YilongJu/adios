...
class cnn_network_contrastive_simplified2(nn.Module):
    ...
    def __init__(self, dropout_type="drop1d", p1=0.1, p2=0.1, p3=0.1, nencoders=1, stride=3, n_classes=2, in_channels=None,
        ...
        self.view_module = nn.Sequential(
            nn.Conv1d(c1, c2, k, s),
            nn.BatchNorm1d(c2),
            nn.ReLU(),
            nn.MaxPool1d(2),
        ...
        self.view_linear_module = nn.Linear(c4 * c4_multiplier, self.embedding_dim)
    ...
    def forward(self, x):
        ...
        x = x.float()
        x = self.view_module(x)  # nencoder = 1 (used for all views)
        x = torch.reshape(x, (x.shape[0], x.shape[1] * x.shape[2]))
        x = self.view_linear_module(x)
        return x
...
if __name__ == "__main__":
    model = cnn_network_contrastive_simplified2(stride=2, embedding_dim=256, kernel_size=7)
