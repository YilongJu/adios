...
class BasicBlock_1D(nn.Module):
    ...
    def __init__(
        ...
        if groups != 1 or base_width != 64:
            raise ValueError('BasicBlock_1D only supports groups=1 and base_width=64')
        if dilation > 1:
            raise NotImplementedError("Dilation > 1 not supported in BasicBlock_1D")
    ...
    def __init__(
        ...
        **kwargs
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, Bottleneck_1D):
                    nn.init.constant_(m.bn3.weight, 0)  # type: ignore[arg-type]
                elif isinstance(m, BasicBlock_1D):
                    nn.init.constant_(m.bn2.weight, 0)  # type: ignore[arg-type]

    def _make_layer(self, block: Type[Union[BasicBlock_1D, Bottleneck_1D]], planes: int, blocks: int,
