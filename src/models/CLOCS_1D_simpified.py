...
class cnn_network_contrastive_simplified(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.float()
        x = self.view_modules[0](x)  # nencoder = 1 (used for all views)
        x = torch.reshape(x, (x.shape[0], x.shape[1] * x.shape[2]))
        x = self.view_linear_modules[0](x)
        return x
...
if __name__ == "__main__":
    model = cnn_network_contrastive_simplified(stride=2, embedding_dim=256, kernel_size=7)
